// Lovable AI Gateway adapter — kept as an optional fallback. Uses the
// OpenAI-compatible endpoint exposed by the gateway.
import {
  type AiProvider,
  type AiChatArgs,
  type AiPlanArgs,
...
async function call(
  ...
  const k = key();
  if (!k) return notConfiguredError("lovable");
...
export const lovableProvider: AiProvider = {
  ...
  async chat(args) {
    ...
    if (!content) return { ok: false, error: "lovable returned empty content." };
  ...
  async streamChat(args) {
    ...
    if (!r.ok) return r;
    if (!r.value.body) return { ok: false, error: "lovable stream missing body" };
  ...
  async plan(args: AiPlanArgs): Promise<AiResult<BuildPlan>> {
    ...
    if (!argStr) return { ok: false, error: "lovable plan missing tool_calls." };
    try {
      ...
      if (!plan.steps.length) return { ok: false, error: "lovable plan has no steps." };
      return { ok: true, value: plan };
    } catch (e) {
      return { ok: false, error: `lovable plan arguments not JSON: ${e}` };
    }
  },
};
