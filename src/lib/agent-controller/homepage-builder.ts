...
<body>
  ...
  <header class="site-header">
    <a class="brand" href="#top" aria-label="${firmName} home">
      <img class="brand-logo brand-logo-full" src="${logoUrl}" alt="${firmName} logo" />
      <span class="brand-name">${firmName}</span>
    </a>
...
.site-header{position:sticky;top:0;z-index:10;display:flex;align-items:center;justify-content:space-between;gap:1.25rem;padding:1rem clamp(1rem,4vw,3rem);background:rgba(247,241,232,.9);backdrop-filter:blur(14px);border-bottom:1px solid var(--line)} .brand{display:flex;align-items:center;gap:.85rem;font-weight:800;color:var(--navy);min-width:min(260px,44vw)} .brand-logo{object-fit:contain;display:block}.brand-logo-full{width:min(220px,42vw);height:56px;object-position:left center;border-radius:0;background:transparent;box-shadow:none}.brand-name{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.site-navigation{display:flex;gap:1.25rem;align-items:center;flex-wrap:wrap;color:var(--muted);font-size:.95rem}.site-navigation a:hover{color:var(--navy)}
...
@media(max-width:900px){.site-header{align-items:flex-start;flex-direction:column}.brand{min-width:0}.brand-logo-full{width:min(180px,70vw);height:48px}.site-navigation{gap:.8rem}.hero{grid-template-columns:1fr}.trust-strip,.cards-grid,.team-grid,.pricing-grid,.testimonial-grid,.process-list{grid-template-columns:1fr}.contact-form{grid-template-columns:1fr}}`;
