...
describe("yawB project security monitor", () => {
  it("passes a clean static homepage baseline", () => {
    const report = scanProjectSecurity([
      {
        path: "index.html",
        content: "<!doctype html><html><head><meta http-equiv=\"Content-Security-Policy\" content=\"default-src 'self'\"></head><body><h1>Hello</h1></body></html>",
      },
  ...
  it("flags insecure HTTP references and external form posts", () => {
    const report = scanProjectSecurity([
      { path: "index.html", content: "<form action=\"https://example.com/lead\"></form><img src=\"http://cdn.example.com/a.png\" />" },
    ], new Date("2026-05-09T00:00:00.000Z"));
