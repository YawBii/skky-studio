...
describe("isFailedJobResolved", () => {
  it("resolves when newer same-type success exists", () => {
    const failed = mkJob({ id: "f1", type: "build.production", status: "failed", createdAt: "2026-05-01T10:00:00Z" });
    const succ = mkJob({ id: "s1", type: "build.production", status: "succeeded", createdAt: "2026-05-01T11:00:00Z" });
  ...
  it("does NOT resolve when failure is newer than success", () => {
    const succ = mkJob({ id: "s1", type: "build.production", status: "succeeded", createdAt: "2026-05-01T10:00:00Z" });
    const failed = mkJob({ id: "f1", type: "build.production", status: "failed", createdAt: "2026-05-01T11:00:00Z" });
  ...
  it("does NOT resolve cross-type (failed ai.plan, succeeded build.production)", () => {
    const failed = mkJob({ id: "f1", type: "ai.plan", status: "failed", createdAt: "2026-05-01T10:00:00Z" });
    const succ = mkJob({ id: "s1", type: "build.production", status: "succeeded", createdAt: "2026-05-01T11:00:00Z" });
    expect(isFailedJobResolved(failed, [failed, succ])).toBe(false);
  });

  it("resolves transient invalid bearer token build.production failure with later build.production success", () => {
    const failed = mkJob({ id: "f1", type: "build.production", status: "failed", createdAt: "2026-05-01T10:00:00Z", error: "invalid bearer token" });
    const succ = mkJob({ id: "s1", type: "build.production", status: "succeeded", createdAt: "2026-05-01T11:00:00Z" });
    expect(isFailedJobResolved(failed, [failed, succ])).toBe(true);
  });
});
...
describe("partitionFailures + grouping", () => {
  it("partitions correctly and groups by type", () => {
    const oldBuildFail = mkJob({ id: "f1", type: "build.production", status: "failed", createdAt: "2026-05-01T08:00:00Z" });
    const buildSucc = mkJob({ id: "s1", type: "build.production", status: "succeeded", createdAt: "2026-05-01T09:00:00Z" });
    const newAiFail = mkJob({ id: "f2", type: "ai.plan", status: "failed", createdAt: "2026-05-01T12:00:00Z" });
...
describe("describeUnresolvedReason", () => {
  it("reports 'no newer success' when no success of that type exists", () => {
    const failed = mkJob({ id: "f1", type: "build.production", status: "failed", createdAt: "2026-05-01T10:00:00Z" });
    expect(describeUnresolvedReason(failed, [failed])).toMatch(/No newer build\.production success/);
  });
