...
export interface SupabasePolicy { table: string; name: string; command: string; using: string }
export interface SupabaseFunction { name: string; language: "sql" | "plpgsql"; security: "definer" | "invoker" }
export interface StorageBucket { name: string; public: boolean; size: string; objects: number }
...
export async function listTables(_projectId: string): Promise<SupabaseTable[]> {
  return [
    ...
    { name: "organizations", rows: 42, rls: true, status: "ok", columns: [{ name: "id", type: "uuid", nullable: false }] },
    { name: "subscriptions", rows: 38, rls: true, status: "ok", columns: [{ name: "id", type: "uuid", nullable: false }] },
    { name: "audit_logs", rows: 9214, rls: false, status: "warn", columns: [{ name: "id", type: "bigint", nullable: false }] },
...
export async function listPolicies(_projectId: string): Promise<SupabasePolicy[]> {
  return [
    ...
    { table: "subscriptions", name: "subs_select_org", command: "SELECT", using: "org_id = current_org()" },
  ];
}
