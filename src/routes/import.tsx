...
function ImportPage() {
  ...
  if (!workspaceIsReal || !projectIsReal || !project) {
    return <NoProjectSelected hint="Select or create a project first, then import a GitHub repo to link to it." />;
  }
  ...
  return (
    <div className="px-6 md:px-10 py-10 max-w-[800px] mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-display font-bold tracking-tight">Link a GitHub repository</h1>
        <p className="mt-2 text-muted-foreground">Paste an owner/repo or full GitHub URL. The link is stored in <code className="font-mono">project_connections</code>; OAuth & sync wire in the next pass.</p>
      </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-gradient-card shadow-elevated p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button variant="hero" size="lg" onClick={link} disabled={!parsed || busy}>
            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Github className="h-4 w-4" />} Link repo <ArrowRight />
          </Button>
        </div>
        {parsed && (
          <div className="mt-3 text-[12px] text-muted-foreground">Parsed as <span className="font-mono text-foreground/80">{parsed.fullName}</span></div>
        )}
        <div className="mt-5 text-[11.5px] text-muted-foreground inline-flex items-start gap-2">
          <Sparkles className="h-3 w-3 mt-0.5" />
          We never claim a repo is connected until status flips to <code className="font-mono">connected</code>. Real OAuth & branch sync arrive next.
        </div>
      </div>
    </div>
  );
