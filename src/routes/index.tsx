...
import { Eye, Code2, Database, Rocket, RefreshCw, Monitor, Tablet, Smartphone, ExternalLink, Play, History, BarChart3, Plus } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {tab === "preview"   && <PreviewPane device={device} setDevice={setDevice} projectName={project?.name} isReal={projectIsReal} />}
        {tab === "code"      && <NotConnected title="Code editor" hint="In-app code editing is not connected to this project yet. Ask yawB in the chat to start editing files." />}
        {tab === "database"  && <NotConnected title="Database" hint="Connect Supabase from Integrations to browse tables and RLS." cta={{ label: "Open Integrations", to: "/connectors" }} />}
        {tab === "analytics" && <NotConnected title="Analytics" hint="Production analytics arrive once Vercel/Plausible is connected." cta={{ label: "Open Integrations", to: "/connectors" }} />}
        ...
        {tab === "history"   && <NotConnected title="History" hint="Connect a Git provider from Integrations to see commit history." cta={{ label: "Open Integrations", to: "/connectors" }} />}
      </div>
    </div>
  );
}
...
function PreviewPane({
  device, setDevice, projectName, isReal,
}: { device: Device; setDevice: (d: Device) => void; projectName?: string; isReal: boolean }) {
